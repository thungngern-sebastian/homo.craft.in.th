<?php
if(!empty($_P['host']) && !empty($_P['ref']) &&!empty($_P['os']) &&!empty($_P['distro']) && !empty($_SESSION["username"])){
    $pdo = _conn();
    ...
    if(!is_array($query) || @!isset($query['failed'])){
        ...
        if(empty($user)){
          ...
          if($is_admin){
              ...
              $query = _que('REPLACE INTO os (ref, os, distro, host) VALUES (?,?,?,?)', [$_P['ref'],$_P['os'],$_P['distro'],$_P['host']]);
              if(!is_array($query) || @!isset($query['failed'])){
