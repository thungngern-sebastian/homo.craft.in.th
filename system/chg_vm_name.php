<?php
if(!empty($_P['ref']) && !empty($_P['name']) && ctype_alnum(str_replace(['-', '_','!','@','.'], '', $_P['name'])) && strlen($_P['name']) <= 20 && !empty($_SESSION["username"])){
    $pdo = _conn();
    ...
    if(!is_array($query) || @!isset($query['failed'])){
            ...
            if(!$is_suspended && $is_activated){
                $query = _que('UPDATE vm SET user_label=? WHERE ref=? AND cusid=?', [$_P['name'],$_P['ref'],$_SESSION["username"]]);
                if(!is_array($query) || @!isset($query['failed'])){
