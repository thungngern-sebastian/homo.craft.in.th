    ...
    if(empty($user)){
       $data = ['msg' => L::data_incorrect];
        ...
        if (hash('sha256', hash('sha256',$_P['password']) . $tmp[2]) == $tmp[3]) {
            ...
            $mail->isHTML(true);
            $mail->Subject = '[Craft.in.th] Login infomation';
            /*
            ...
            <center>
                <div style="color:#111;padding:2rem;background-color:#fff;width:100%;max-width:350px;">
                    <p>สวัสดี</p>
                    <p>เราได้ส่ง Email นี้มา เพราะ มี คนใหม่เข้า บัญชี โดย</p>
            ...
            */
            $email_text = ['user/login.png', $user['fname'].' '.$user['lname'], 'Login infomation', 'เราได้ส่ง Email นี้มาเพื่อแจ้งเตือนการเข้าสู่ระบบ โดย', 'IP '.$ip.'<br>Country '.$country.'<br>Region '.$rg.'<br>City '.$city.'<br>หากไม่ใช่คุณ กรุณาติดต่อเราโดยทันที เพื่อป้องกันมิจฉาชีพที่จะมาสวมรอยเป็นคุณ', 'ติดต่อเรา', 'https://www.facebook.com/craftstudioofficial'];
            ...
            $res_alt = 'สวัสดี เราได้ส่ง Email นี้มา เพราะ มี คนใหม่เข้า บัญชี โดย IP '.$ip.' Country '.$country.' Region '.$rg.' City '.$city.' หากไม่ กรุณาติดต่อเราโดยทันที เพื่อป้องกันมิจฉาชีพที่จะมาสวมรอยเป็นคุณ'.$end_body;
            
            $mail->addAddress($user['email']);
            $mail->Body    = $res;
            $mail->AltBody = $res_alt;
            _que('INSERT INTO log (data) VALUES (?)',["Login to {$user['email']} via E-mail {$ip} {$country} {$rg} {$city}"]);
            // $mail->send();
            if (!$mail->send()) {
